...
namespace ServiceStack.Server.Tests.AuthRepo
{
    ...
    public class Neo4jAuthRepositoryTests
    {
        ...
        protected IUserAuthRepository Sut { get; set; }

        public Neo4jAuthRepositoryTests()
        {
            appHost = new BasicAppHost().Init();
            appHost.Container.AddSingleton(GraphDatabase.Driver("bolt://localhost:7687"));
            appHost.Container.AddTransient<IUserAuthRepository>(container =>
                new Neo4jAuthRepository<AppUser, UserAuthDetails>(container.Resolve<IDriver>()));
        }
        ...
        public void SaveUserAuth_Should_Throw_When_UserAuthId_Is_Not_An_Integer()
        {
            ...
            // Assert
            exception.Message.Should().Be("Cannot convert to integer (Parameter 'userAuthId')");
        }
